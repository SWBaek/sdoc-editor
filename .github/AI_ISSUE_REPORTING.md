...
| Reproducible VS Code extension problem | VS Code bug report |
...
- Add `area: vscode` for behavior specific to the VS Code extension. The
  Windows Desktop app reached end of life with v0.7.8; Desktop-only reports are
  outside the supported scope unless the same cause affects current shared
  code or a supported delivery surface.
