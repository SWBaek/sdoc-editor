...
- Use Grok CLI in two independent modes: a planning critique before judgment-heavy work and a final-diff critique after material implementation. Invoke `.agents/skills/orchestrate-sdoc-work/scripts/invoke-advisor.ps1` directly for single-stream work; use `$orchestrate-sdoc-work` when its multi-workstream or delegation triggers apply.
- Request a planning critique before the first consequential write or decision when the task requires product interpretation, an interaction-model choice, architecture or persistence design, security or data-loss reasoning, cross-host behavior, or another assumption that could materially change scope or direction. Ask Grok to challenge the proposed approach, surface false assumptions and counterexamples, and suggest simpler alternatives within the user's explicit constraints.
- Skip the planning critique for typo or wording polish, release/version mechanics, pure renames, routine dependency maintenance without behavior changes, verification-only work whose direction is already settled, and clear single-cause fixes that require no product or architectural judgment. Re-run planning critique only when the plan changes materially, not for every incremental implementation choice.
- A planning critique never satisfies the final-diff requirement. Every material implementation and substantive repository verification still requires an independent final Grok critique after local checks and final-diff inspection, even when the user did not explicitly request cross-model review.
- Do not load a full diff or project bundle into agent conversation context, `-Prompt`, or a shell command line. For required critiques, pass a small UTF-8 task specification to the wrapper and select `Planning` or `FinalDiff`; the wrapper and its context generator assemble, hash, validate, and clean up the bounded temporary review bundle.
- A Grok exit code of 0 is not sufficient evidence of review. The wrapper must validate the mode-specific versioned contract and return `reviewStatus: pass` or `reviewStatus: changes_required`; `incomplete`, `unavailable`, `diagnostic`, acknowledgement-only, contradictory, or evidence-free output never counts as the required critique. Summary output is compact presentation; use `-ResultFile` for the validated full result. Legacy/Text output is compatibility-only and never weakens required-review validation.
- Planning task specifications contain the bounded goal, proposed approach, alternatives, assumptions, affected surfaces, and open questions. Final-diff task specifications contain requirements, acceptance criteria, and relevant test evidence; the generator selects the exact change set and canonical repository context. If Planning requests more context, only the host-side generator may expand an allowlisted bundle and resubmit it once. Grok receives payload-only input and must not use shell, repository-read, web, subagent, or other tools. Never send secrets, credentials, unrelated proprietary content, personal information, customer data, or sensitive logs.
- Treat Grok output as adversarial advice, not approval, veto, or consensus. The main agent owns the decision: disposition planning findings with explicit rationale and update the issue or plan when product-facing; disposition final findings with repository or test evidence. If reviewed files change materially, run the final critique again before completion.
- If Grok CLI is unavailable, unauthenticated, or cannot be safely scoped, report the missing Grok critique and residual risk explicitly; never imply that Grok reviewed the change. Other external advisors such as agy remain opt-in unless the user requests them.
- Invoke required Grok critiques with the explicit `Planning` or `FinalDiff` contract and keep structured validation enabled. The default `Legacy` contract and `-DiagnosticMode` (including its deprecated `-AllowIncompleteResponse` alias) are compatibility/connectivity surfaces only and must never satisfy a required planning or final-diff critique.
- Required critiques use an available direct `grok-*` model reported by the CLI; do not satisfy this policy through an `ocx-*` model alias or another provider.
