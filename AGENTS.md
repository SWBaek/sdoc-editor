...
- Never embed a full diff or another long review payload in `-Prompt` or a shell command line. Write the bounded review payload to an OS temporary UTF-8 file, invoke the wrapper with `-PromptFile <path>`, and remove the caller-owned temporary file in `finally`. The wrapper uses and deletes its own managed Grok prompt file.
- A Grok exit code of 0 is not sufficient evidence of review. The wrapper must validate its versioned JSON contract and return `reviewStatus: pass` or `reviewStatus: changes_required`; `incomplete`, `unavailable`, `diagnostic`, acknowledgement-only, contradictory, or evidence-free output never counts as the required critique. `-OutputFormat Text` is display-only compatibility and its headings or `NO_ACTIONABLE_FINDINGS` token are not validation evidence.
- For planning critique, give Grok the bounded goal, proposed approach, alternatives, assumptions, affected surfaces, and open questions; no diff or test log is required. For final critique, give it only the bounded final diff, requirements, acceptance criteria, and relevant test evidence. Never send secrets, credentials, unrelated proprietary content, personal information, customer data, or sensitive logs.
...
- Keep normal Grok calls on the wrapper's default structured output. `-DiagnosticMode` (including its deprecated `-AllowIncompleteResponse` alias) proves connectivity only and must never be used for a required planning or final-diff critique.
