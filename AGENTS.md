...
- A Grok exit code of 0 is not sufficient evidence of review. Require a substantive report with explicit ASCII `Conclusion` and `Findings` headings or `NO_ACTIONABLE_FINDINGS`; an acknowledgement, intent statement, or other incomplete response must be retried and must not count as the required critique.
