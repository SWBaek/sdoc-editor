...
### Fixed
- Prevented reduced-motion accessibility styles from altering Mermaid HTML-label geometry while preserving motion reduction for editor-owned controls (#227)
...
### Fixed
- Preserved intrinsic SVG dimensions for Mermaid, D2, Graphviz, PlantUML, and other diagram renderers so diagrams are inserted at a more appropriate size instead of inheriting oversized responsive dimensions (#227)
...
### Fixed
- Prevented Ctrl+S save lifecycle events without content changes from being reported as external document edits, eliminating the repeated conflict and comparison loop
- Reconciled host-confirmed modified timestamps with queued editor snapshots so successful disk saves settle on `Saved` without redundant persistence mutations
...
### Changed
- Simplified CLI onboarding around the exact `sdoc-editor-cli` package name, project scripts for repeatable automation, and optional global `sdoc` usage

## [0.9.2] - 2026-08-07

### Fixed
- Normalized npm package metadata so the public Registry preserves the `sdoc` executable during publication
...
### Fixed
- Restored the persisted Interface language selection when VS Code cannot write User Settings
- Added an actionable **Open User Settings** recovery path without interrupting the editor message queue
...
### Changed
- Marked the template workflow as experimental in the VS Code command, both host dialogs, and user documentation
